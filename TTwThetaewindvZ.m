%%TTwwindvZ
    ...
    %
    %General form: [foundit] = TTwThetaevZ(y,m,d,t,sounding,kmTop)
    %
    %Output:
...
function [foundit] = TTwThetaewindvZ(y,m,d,t,sounding,kmTop)
...
useRH = sounding(foundit).rhum(kmCutoff==1);
...
% Convert units
tempK = useTemp+273.15; %Celsius to Kelvin

[qvOut] = qv(tempK,useRH,usePressure.*100);
[thetae] = theta_e(usePressure.*100,tempK,qvOut);

axC = gca;
axC_pos = axC.Position;
axK = axes('Position',axC_pos,'XAxisLocation','bottom','YAxisLocation','right','Color','none');
thetae_profile = line(thetae,useHeight,'Parent',axK,'Color',[16 84 86]./255);
thetae_profile.LineWidth = 2.4;
axK.FontName = 'Lato Bold'; axK.FontSize = 18;
